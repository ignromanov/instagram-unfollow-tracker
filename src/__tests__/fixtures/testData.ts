...
export const ALL_TEST_ACCOUNTS = [
  ...
  'alice_mutual', 'bob_mutual', 'charlie_mutual',
  
  // Following but not followed back
  'dave_unfollowed', 'eve_unfollowed', 'frank_unfollowed',
  
  // Followers but not following back
  'grace_follower', 'henry_follower', 'ivy_follower',
  
  // Pending requests (mixed scenarios)
  'jack_pending', 'kate_pending',
  
  // Permanent requests
  'leo_permanent', 'mia_permanent',
  
  // Restricted profiles
  'nick_restricted', 'olivia_restricted',
  
  // Close friends
  'paul_close', 'quinn_close',
  
  // Recently unfollowed
  'rachel_unfollowed', 'steve_unfollowed',
  
  // Dismissed suggestions
  'tom_dismissed', 'una_dismissed',
] as const;
...
export const TEST_ACCOUNTS: TestAccounts = {
  ...
  notFollowingBack: ['dave_unfollowed', 'eve_unfollowed', 'frank_unfollowed', 'rachel_unfollowed', 'steve_unfollowed'],
...
export function createTestParsedData(): ParsedAll {
  ...
  const pendingSent = new Map(
    TEST_ACCOUNTS.pending.map(username => [username, currentTimestamp])
  );
  ...
  const closeFriends = new Map(
    TEST_ACCOUNTS.close.map(username => [username, currentTimestamp])
  );
